...
(defun insert-some-cds ()
  (setf *db* ())
  (add-record (make-cd 'holy 'jesus 10 t))
...
(defun update
  ...
  (setf *db*
	(mapcar #'(lambda (row)
		    ...
		    row)		;we return the row (which can
					;be modified or not.
		*db*)))
...
(defmacro backwards (sequence)
  (reverse sequence))

(defun fun-that-invokes-backwards ()
  (backwards ("Hello, World!" t format)))

;; what we want to generate is: (equal (getf cd field) value)
